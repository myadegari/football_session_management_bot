...
class CallbackHandler:
    ...
    def _register_handlers(self) -> Dict[str, Callable]:
        ...
        return {
            ...
            "SESSION_DATE_": lambda call, db: self.user_flow.session_date(call, db),
            "ADMIN_START": lambda call, db: self.admin_flow.start(call),
            "ADMIN_CHANGE_BASED_COST": lambda call, db: self.admin_flow.change_based_cost(call, db),
            "ADMIN_GENERATE_REPORT": lambda call, db: self.admin_flow.generate_report(call, db),
            "ADMIN_SESSION_DATE_": lambda call, db: self.admin_flow.seesion_date(call, db),
            "ADMIN_MANAGE_SESSION_": lambda call, db: self.admin_flow.manage_session(call, db),
            "ADMIN_SESSION_REFUND_": lambda call, db: self.admin_flow.session_refund(call, db),
            "ADMIN_DEACTIVATE_SESSION_": lambda call, db: self.admin_flow.deactive_session(call, db),
            "ADMIN_ACTIVATE_SESSION_": lambda call, db: self.admin_flow.active_session(call, db),
            "BOOK_": lambda call, db: self.user_flow.book_session(call, db),
            "CONFIRM_": lambda call, db: self.user_flow.confirm_session(call, db),
            "ADMIN_VIEW_USER_BOOKINGS_FROM_": lambda call, db: self.admin_flow.view_user_bookings(call, db),
            "ADMIN_VIEW_USER_": lambda call, db: self.admin_flow.view_user_details(call, db),
            "ADMIN_VIEW_USERS_PAGE_": lambda call, db: self.admin_flow.view_users(call, db),
            "ADMIN_VIEW_SESSIONS": lambda call, db: self.admin_flow.view_sessions(call, db),
            "ADMIN_GENERATE_SESSIONS": lambda call, db: self.admin_flow.generate_sessions(call, db),
            "REPORT_ALL_PAYMENTS": lambda call, db: self.user_flow.report_all_payment(call, db),
            "ADMIN_CHANGE_BASED_COST_": lambda call, db: self.admin_flow.change_cost(call, db),
        }
    ...
    def handle(self, call: telebot.types.CallbackQuery, db: Session) -> bool:
        ...
        for prefix, handler in self.handlers.items():
            if call.data == prefix or (prefix.endswith("_") and call.data.startswith(prefix)):
                handler(call, db)
                return True
        return False
...
class FootballSessionBot:
    ...
    def register_handlers(self) -> None:
        ...
        @inject
        def callback_center(call: telebot.types.CallbackQuery, db: Session = Dependency(get_db)):
            self.callback_handler.handle(call, db)
        
        @self.bot.message_handler(func=lambda message: True)
        @inject
        def message_center(message: telebot.types.Message, db: Session = Dependency(get_db)):
